  ...
  template: `
    <div class="min-h-screen pt-28 pb-24 px-[max(24px,5vw)] relative">
      ...
      <div class="max-w-7xl mx-auto">
        <div class="mb-10">
          <span class="text-[11px] font-semibold tracking-[0.15em] uppercase text-on-surface-variant px-4 py-1 rounded-full glass-panel inline-block mb-4">Admin Panel</span>
          <h1 class="text-[48px] leading-[1.2] tracking-[-0.02em] font-light text-on-surface">Users</h1>
          @if (usersPage()) {
            <p class="text-base text-on-surface-variant mt-2">{{ usersPage()!.totalElements }} total members</p>
          }
        </div>
                ...
                <table class="w-full">
                  <thead>
                    <tr class="border-b border-outline-variant/30">
                      <th class="px-6 py-4 text-left text-[11px] font-semibold tracking-widest uppercase text-on-surface-variant">User</th>
                      <th class="px-6 py-4 text-left text-[11px] font-semibold tracking-widest uppercase text-on-surface-variant">Email</th>
                      <th class="px-6 py-4 text-left text-[11px] font-semibold tracking-widest uppercase text-on-surface-variant">Role</th>
                      <th class="px-6 py-4 text-left text-[11px] font-semibold tracking-widest uppercase text-on-surface-variant">Joined</th>
                      <th class="px-6 py-4 text-left text-[11px] font-semibold tracking-widest uppercase text-on-surface-variant">Status</th>
                        ...
                        <td class="px-6 py-4">
                          @if (user.verified) {
                            ...
                            <span class="flex items-center gap-1 text-xs text-on-surface-variant/60">
                              <span class="material-symbols-outlined" style="font-size:14px">pending</span> Unverified
                            </span>
                          }
                        </td>
...
export class AdminUserListComponent implements OnInit {
  ...
  deleteUser(user: UserResponseDto): void {
    ...
    this.userService.delete(user.id).subscribe({
      next: () => {
        this.toast.show('Kullanıcı silindi.', 'success');
